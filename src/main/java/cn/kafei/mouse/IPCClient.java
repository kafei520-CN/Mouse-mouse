            ...
            for (int i = 0; i < 40 && running; i++) {
                try {
                    ...
                    if (!ack.startsWith("ACK:")) {
                        Mousemouse.LOGGER.warn("Device {} rejected: {}", deviceId, ack);
                        socket.close();
                        return;
                    }
                    Mousemouse.LOGGER.info("Claimed device {}", deviceId);
                    receiveLoop(in);
                    return;
                } catch (ConnectException e) {
                    try { Thread.sleep(250); } catch (InterruptedException ie) { return; }
                } catch (IOException e) {
                    if (running) Mousemouse.LOGGER.warn("IPC error device {}: {}", deviceId, e.getMessage());
                    return;
                }
            }
        }
